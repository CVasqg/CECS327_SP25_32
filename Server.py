...
def handle_query(query, cursor):
    try:
        ...
        if "average moisture" in query:
            ...
            return f"Average moisture (from Moisture Meter - moisture meter sensor): {result:.2f} % RH" if result else "No data found."

        # Query for average water consumption from water sensor
        elif "average water consumption" in query:
            print("→ Running water query...")
            cursor.execute('''
                SELECT 
            ...
            return f"Average water consumption: {result:.2f} gallons" if result else "No data found."
